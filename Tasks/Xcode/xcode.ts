    ...
    try {
        ...
        if(tl.getBoolInput('packageApp', true) && sdk !== 'iphonesimulator') {
            if (useXctool || !ws || !scheme || xcodeVersion < 7) {
                ...
                exportPath = tl.resolve(exportPath, scheme + '.ipa');
                ...
                var exportOptions = tl.getInput('exportOptions');
                var exportOptionsPlist : string;
                if(exportOptions === 'specify') {
                    var exportMethod : string = tl.getInput('exportMethod');
                    ...
                    if(!tl.filePathSupplied('exportOptionsPlist') || !tl.exist(exportOptionsPlist)) {
                        throw tl.loc('ExportOptionsPlistInvalidFilePath', exportOptionsPlist);
                    }
                } else {
                    tl.warning(tl.loc('ExportOptionsNotSpecified'));
                }
